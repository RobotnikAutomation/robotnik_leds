...
public:
    ...
    void insertPart (TYPE_PART part, int led_start, int led_end, Color color1 , Color color2 = Color(TYPE_COLOR::black), Color color3 = Color(TYPE_COLOR::black));
    void insertPart (TYPE_PART part, int led_start, int led_end, TYPE_COLOR color1 , TYPE_COLOR color2 = TYPE_COLOR::black, TYPE_COLOR color3 = TYPE_COLOR::black);
    void inserEffect (TYPE_PART part, TYPE_LED_MODE mode, Color color1 , Color color2 = Color(TYPE_COLOR::black), Color color3 = Color(TYPE_COLOR::black));
    void inserEffect (TYPE_PART part, TYPE_LED_MODE mode, TYPE_COLOR color1, TYPE_COLOR color2 = TYPE_COLOR::black, TYPE_COLOR color3 = TYPE_COLOR::black);
...
private:
    ...
    robotnik_leds::leds_value zone(TYPE_PART part, int led_start, int led_end, int numColor=1);
    ...
    vector <robotnik_leds::leds_value> cumulative(TYPE_PART _part, bool inverse=false);
    vector <robotnik_leds::leds_value> cumulativeCenter(TYPE_PART _part, bool inverse=false);
    vector <robotnik_leds::leds_value> blinkingEffect(TYPE_PART _part);
    vector <robotnik_leds::leds_value> round(TYPE_PART _part, bool inverse= false);
