...
#ifndef __SERIALDEV_H_
#define __SERIALDEV_H_
...
class SerialDevice {
   ...
   public:
    ...
    SerialDevice(void);
    // Public Constructor
    SerialDevice(const char *device, int baudrate, const char *parity,
                 int datasize);
    // Public Destructor
    ~SerialDevice(void);
    int OpenPort1(void);
    ...
    int WritePort(char *chars, int *written_bytes,
                  int length);  // Importat del SerialDevice amb Component
    int ReadPort(char *result);
    int ReadPort(char *result, int num_bytes);
    int ReadPort(char *result, int *read_bytes,
                 int num_bytes);  // Importat del SerialDevice amb Component
    //! Clean port buffer
    int Flush();
