...
LedsEffects::LedsEffects(int _leds, int _period) {
    ...
    move_cont = 0;
    LedsPart aux_all =
        LedsPart(TYPE_PART::all, 0, leds, TYPE_LED_MODE::none, TYPE_COLOR::red);
...
LedsEffects::LedsEffects(map<TYPE_PART, LedsPart> _parts, int _leds,
                         int _period) {
    leds = _leds;
    period = _period;
    clock = 1;
    ...
    for (map<TYPE_PART, LedsPart>::iterator it = _parts.begin();
         it != _parts.end(); ++it) {
    ...
    if (!have_part_all) {
        LedsPart aux_all = LedsPart(TYPE_PART::all, 0, leds,
                                    TYPE_LED_MODE::none, TYPE_COLOR::red);
        aux_all.setEndable(false);
        parts.insert(pair<TYPE_PART, LedsPart>(TYPE_PART::all, aux_all));
    }
    init = false;
...
void LedsEffects::setPeriod(int _period) { period = _period; }
...
void LedsEffects::insertPart(TYPE_PART part, int led_start, int led_end,
                             Color color1, Color color2, Color color3) {
    parts.insert(pair<TYPE_PART, LedsPart>(
        part, LedsPart(part, led_start, led_end, TYPE_LED_MODE::none, color1,
                       color2, color3)));
}

void LedsEffects::insertPart(TYPE_PART part, int led_start, int led_end,
                             TYPE_COLOR color1, TYPE_COLOR color2,
                             TYPE_COLOR color3) {
    parts.insert(pair<TYPE_PART, LedsPart>(
        part, LedsPart(part, led_start, led_end, TYPE_LED_MODE::none,
                       Color(color1), Color(color2), Color(color3))));
}

void LedsEffects::inserEffect(TYPE_PART part, TYPE_LED_MODE mode, Color color1,
    ...
    if (it != parts.end()) {
        ...
        cout << "Error - leds effects: You don't insert the part " << part
             << endl;
    }
}

void LedsEffects::inserEffect(TYPE_PART part, TYPE_LED_MODE mode,
                              TYPE_COLOR color1, TYPE_COLOR color2,
                              TYPE_COLOR color3) {
    map<TYPE_PART, LedsPart>::iterator it = parts.find(part);
    if (it != parts.end()) {
...
void LedsEffects::disableAllEffect() {
    for (map<TYPE_PART, LedsPart>::iterator it = parts.begin();
         it != parts.end(); ++it) {
...
LedsPart LedsEffects::getLedPart(TYPE_PART part) {
    ...
    if (it != parts.end()) {
        ...
        cout << "Error - leds effects: You don't' insert the part " << part
             << endl;
        return LedsPart(TYPE_PART::none, 0, 0, TYPE_LED_MODE::none,
...
void LedsEffects::setPaintedZone(TYPE_PART _part, bool _painted) {
    ...
    if (it != parts.end()) {
        ...
        cout << "Error - leds effects: You don't' insert the part " << _part
             << endl;
    }
}
...
robotnik_leds::leds_value LedsEffects::zone(TYPE_PART part, int led_start,
                                            int led_end, int numColor) {
    robotnik_leds::leds_value zone_msg;
    zone_msg.request.mode = 'z';
    zone_msg.request.ledStart = parts.at(part).getLedStart() +
                                led_start % (parts.at(part).getLedEnd() + 1);
    zone_msg.request.ledEnd = parts.at(part).getLedStart() +
                              led_end % (parts.at(part).getLedEnd() + 1);
    if (numColor == 1) {
        zone_msg.request.white = parts.at(part).getColor1().w;
        zone_msg.request.red = parts.at(part).getColor1().r;
        zone_msg.request.green = parts.at(part).getColor1().g;
...
robotnik_leds::leds_value LedsEffects::pixel(TYPE_PART part, int led,
                                             int numColor) {
    robotnik_leds::leds_value pixel_msg;
    pixel_msg.request.mode = 'p';
    pixel_msg.request.ledStart =
        parts.at(part).getLedStart() + led % (parts.at(part).getLedEnd() + 1);
    if (numColor == 1) {
...
vector<robotnik_leds::leds_value> LedsEffects::clearZoneEffect(
    TYPE_PART _part) {
...
vector<robotnik_leds::leds_value> LedsEffects::continuousEffect(
    TYPE_PART _part) {
    vector<robotnik_leds::leds_value> vsrv;
...
vector<robotnik_leds::leds_value> LedsEffects::round(TYPE_PART _part,
                                                     bool inverse) {
    vector<robotnik_leds::leds_value> vsrv;
    if (!parts.at(_part).getPainted()) {
        vsrv.push_back(zone(_part, TYPE_COLOR::black));
...
vector<robotnik_leds::leds_value> LedsEffects::cumulative(TYPE_PART _part,
                                                          bool inverse) {
    vector<robotnik_leds::leds_value> vsrv;
    if (!parts.at(_part).getPainted()) {
        vsrv.push_back(zone(_part, TYPE_COLOR::black));
    ...
    if ((clock_frame) % period == 0) {
        ...
        if (inverse) {
            ...
            color1 = 2;
            color2 = 1;
            vsrv.push_back(zone(_part, led + 1,
                                parts.at(_part).getLedEnd() - led, color2));
            vsrv.push_back(zone(_part, 0, led, color1));

        } else {
            vsrv.push_back(zone(_part, led + 1,
                                parts.at(_part).getLedEnd() - led, color1));
...
vector<robotnik_leds::leds_value> LedsEffects::cumulativeCenter(TYPE_PART _part,
                                                                bool inverse) {
    vector<robotnik_leds::leds_value> vsrv;
    if (!parts.at(_part).getPainted()) {
        vsrv.push_back(zone(_part, TYPE_COLOR::black));
...
vector<robotnik_leds::leds_value> LedsEffects::knightRiderEffect(
    TYPE_PART _part) {
    vector<robotnik_leds::leds_value> vsrv;
    if (!parts.at(_part).getPainted()) {
        vsrv.push_back(zone(_part, TYPE_COLOR::black));
...
vector<robotnik_leds::leds_value> LedsEffects::doubleKnightRiderEffect(
    TYPE_PART _part) {
    vector<robotnik_leds::leds_value> vsrv;
    if (!parts.at(_part).getPainted()) {
        vsrv.push_back(zone(_part, TYPE_COLOR::black));
...
vector<robotnik_leds::leds_value> LedsEffects::callEffect(TYPE_PART _part) {
    switch (parts.at(_part).getMode()) {
        ...
        case TYPE_LED_MODE::blinking:
            if (isActivePart(_part)) return blinkingEffect(_part);
            break;
        case TYPE_LED_MODE::round:
            if (isActivePart(_part)) return round(_part);
            break;
        case TYPE_LED_MODE::roundInv:
            if (isActivePart(_part)) return round(_part, true);
            break;
        case TYPE_LED_MODE::knightRider:
            if (isActivePart(_part)) return knightRiderEffect(_part);
            break;
        case TYPE_LED_MODE::doubleKnightRider:
            if (isActivePart(_part)) return doubleKnightRiderEffect(_part);
            break;
        case TYPE_LED_MODE::cumulative:
            if (isActivePart(_part)) return cumulative(_part);
            break;
        case TYPE_LED_MODE::cumulativeInv:
            if (isActivePart(_part)) return cumulative(_part, true);
            break;
        case TYPE_LED_MODE::cumulativeCenter:
            if (isActivePart(_part)) return cumulativeCenter(_part);
            break;
        case TYPE_LED_MODE::cumulativeCenterInv:
            if (isActivePart(_part)) return cumulativeCenter(_part, true);
            break;
    }
...
vector<robotnik_leds::leds_value> LedsEffects::paint(bool increment_clock) {
    ...
    if (isActivePart(TYPE_PART::all)) {
        ...
        for (map<TYPE_PART, LedsPart>::iterator it = parts.begin();
             it != parts.end(); ++it) {
            if (it->second.getPart() != TYPE_PART::all &&
                isActivePart(it->first)) {
                aux.empty();
